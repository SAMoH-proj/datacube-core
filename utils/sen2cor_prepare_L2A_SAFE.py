...
def prepare_dataset(path):
    ...
    granuleslist = [(granule.get('granuleIdentifier'), [imid.text for imid in granule.findall('IMAGE_FILE_2A')]) for
                    granule in
                    root.findall('./*/L2A_Product_Info/L2A_Product_Organisation/Granule_List/Granule')]
    current = 0
    list = []
    granules = {}
    ...
    for granule_id, images in granules.items():
        ...
        for image in images:
            ...
            for item in classification_list:
                if item in image:
                    ...
                    if '20m' in image:
                        images_classification.append(os.path.join(image + ".jp2"))

            for item in ten_list:
                if item in image:
                    images_ten_list.append(os.path.join(image + ".jp2"))
                    grouped_images.append(os.path.join(image + ".jp2"))
            for item in twenty_list:
                if item in image:
                    images_twenty_list.append(os.path.join(image + ".jp2"))
                    grouped_images.append(os.path.join(image + ".jp2"))
            for item in sixty_list:
                if item in image:
                    images_sixty_list.append(os.path.join(image + ".jp2"))
                    grouped_images.append(os.path.join(image + ".jp2"))
        ...
        spectral_dict = {image[-11:-4]: {'path': str(path.parent.joinpath(image)), 'layer': 1, } for image in grouped_images}
        scl_dict = {'SCL_20m': {'path': str(path.parent.joinpath(classification)), 'layer': 1, } for classification in
                    images_classification}
...
def main(datasets, output):
    ...
    for dataset in datasets:
        ...
        if path.is_dir():
            print(path)
            if path.joinpath('MTD_MSIL2A.xml').exists():
                path = path.joinpath('MTD_MSIL2A.xml')
        else:
