...
def prepare_dataset(path):
    ...
    product_type = 'S2' + tileInfo['datastrip']['id'].split('_')[2] + level[1:] + 'p'
    ...
    # names are as defined in s2_granules.yaml
    images = ['B01_60m', 'B02_10m', 'B02_20m', 'B02_60m',
                   'B03_10m', 'B03_20m', 'B03_60m', 'B04_10m',
                   'B04_20m', 'B04_60m', 'B05_20m', 'B05_60m',
                   'B06_20m',  'B06_60m', 'B07_20m','B07_60m',
                   'B08_10m', 'B8A_20m', 'B8A_60m', 'B09_60m',
                   'B11_20m', 'B11_60m', 'B12_20m', 'B12_60m',
                   'SCL_20m']
    
    return {
        'id': str(uuid.uuid5(uuid.NAMESPACE_URL, path)),
        'processing_level': level.replace('L','Level-'),
        'product_type': product_type,
        # this is not true but will help us with legacy tools
        ...
        'dataCoverage': (tileInfo['dataCoveragePercentage'] / 100.0) if 'dataCoveragePercentage' in tileInfo else 0,
        'cloudCoverage': tileInfo['cloudyPixelPercentage'] / 100.0,
        'image': {
            'bands': {
                image: {
                    # convert image name to path e.g. B01_60m --> R60m/B01.jp2
                    'path': "R{}/{}.jp2".format(image[-3:], image[:3]),
                    'layer': 1,
                } for image in images
            }
        },
